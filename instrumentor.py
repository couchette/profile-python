...
class Instrumentor:
    ...
    def begin_session(self, name, filepath="results.json"):
        if self.__current_session:
            print(f"Instrumentor.begin_session('{name}') when session '{self.__current_session.Name}' already open.")
...
class InstrumentationTimer:
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        if PROFILE:
            elapsed_time_count = int(time.perf_counter()* 1000000) - self.__start_time_point
            Instrumentor.get().write_profile((self.__name, self.__start_time_point, elapsed_time_count, threading.get_ident()))
        pass
...
class InstrumentorUtils:
    ...
    def cleanup_output_string(expr: str, remove: str) -> str:
        ...
        while src_index < len(expr):
            ...
            while (match_index < len(remove) - 1 and
                src_index + match_index < len(expr) - 1 and
